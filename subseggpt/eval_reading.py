"""
Reading times evaluator for SubSegGPT.

The BabyLM eval pipeline relies on vocab logits to compute word-level surprisal. SubSegGPT 
does not compute logits - it computes marginal probabilities over character sequences. 
We adapt the BabyLM reading pipeline to use word-level surprisal for SubSegGPT.

This script copies the BabyLM reading pipeline
github.com/babylm-org/babylm-eval/blob/main/strict/evaluation_pipeline/reading/run.py
making only two changes (marked #CHANGE) to load our model and conditional probability function. 

Usage: Invoked via run_eval_subseggpt.py reading ... with the same args as the BabyLM pipeline.
"""

from subseggpt.word_surprisal import word_prob, load_model_and_tokenizer
...
def parse_args():
    ...
    parser.add_argument("--model_name", default=None, type=str, help="Override the results folder name (default: basename of the model path).")
    ...
    args.model_name = args.model_name or pathlib.Path(args.model_path_or_name).name  # explicit override, else basename (.name keeps dotted run dirs intact)
...
if __name__ == "__main__":
    ...
    # CHANGED: load our model and tokenizer
    model, tokenizer = load_model_and_tokenizer(args.model_path_or_name, args.revision_name)

    # CHANGED: score P(word|context) with SubSegGPT's conditional marginal.
    p2_function = word_prob
