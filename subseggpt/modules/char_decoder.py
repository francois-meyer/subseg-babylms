...
class LSTMCharDecoder(nn.Module):
    ...
    def __init__(
        ...
        context_size,  # Transformer backbone output size (character history context)
        char_embed_dim=128,
        num_layers=1,
        dropout=0.1,
        seg_end_token_id=3,
    ...
    def init_hidden(self, context):
        ...
        Args:
            context: (batch_size, context_size) output embedding from backbone transformer
    ...
    def forward(self, context, input_ids, target_ids):
        ...
        not the original batch size of the SubSegGPT model training.
