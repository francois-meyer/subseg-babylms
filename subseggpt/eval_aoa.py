"""
Age-of-Acquisition (AoA) evaluator for SubSegGPT.
...
We adapt the BabyLM AoA pipeline to use word-level surprisal for SubSegGPT.
...
Usage: Invoked via run_eval_subseggpt.py aoa ... with the same args as the BabyLM pipeline.
"""
...
def get_checkpoint_steps(model_dir, expected_word_counts):
    ...
    local checkpoints during development or on Hub checkpoints submitted for the challenge.
        1. First check if epoch_<label> dirs are available locally (as saved by SubSegGPT training script).
...
class SubSegSurprisalExtractor(StepSurprisalExtractor):
    """StepSurprisalExtractor that loads our model, tokeniser, and uses SubSegGPT surprisal function."""
    ...
    def compute_surprisal(self, model, processor, tokenizer, context, target_word, use_bos_only=True):
        return -word_logprob(model, tokenizer, context, target_word)
